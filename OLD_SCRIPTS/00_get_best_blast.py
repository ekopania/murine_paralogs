...
orthfile = "/mnt/beegfs/gt156213e/murinae-seq/02-Annotation/mm10-rnor6-master-transcript-id-table.tab";
# The ortholog file between mouse and rat.
orth_tids = {};
first = True;
for line in open(orthfile):
...
for line in open(args.blastfile):
    ...
    query_tid = line[0].split("|")[1]; #query_tid = set(line[0].split("|")[1].split(";"));
    query_eid = line[0].split("|")[3]; #2
    ...
    #target_tid_orth = list(target_tids.intersection(rat_tids)); #NOT checking if target in rat IDs for this version
    ...
    hit = [query_tid, target_id, aln_len, evalue, bit];
    #Changed this from Gregg's original script such that dict keys are a unique combination of exon ID and target (species|paralog)
    query_target = query_eid + "_" + target_id
    query_hits[query_target].append(hit);
